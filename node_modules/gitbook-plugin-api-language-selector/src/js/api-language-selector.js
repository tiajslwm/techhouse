require(["gitbook", "jquery"], function(gitbook, $) {
  var buttonsId = [],
    $codes,
    apiLanguageSelector;

  // Default themes
  var THEMES = [
    {
      config: "light",
      text: "Light",
      id: 0
    },
    {
      config: "dark",
      text: "Dark",
      id: 3
    }
  ];

  // Instantiate localStorage
  function init(config) {
    apiLanguageSelector = gitbook.storage.get("apiLanguageSelector", {
      currentLang: null
    });
  }

  // Update localStorage settings
  function saveSettings() {
    gitbook.storage.set("apiLanguageSelector", apiLanguageSelector);
    updateDisplay();
  }

  // Update display
  function updateDisplay() {
    // Update code samples elements
    $codes = $(".api-method-sample");
    // Display corresponding code snippets
    $codes.each(function() {
      // Show corresponding
      var hidden = !($(this).data("lang") == apiLanguageSelector.currentLang);
      $(this).toggleClass("hidden", hidden);
    });
  }

  // Update code tabs
  function updateCodeTabs() {
    // Remove languages buttons
    gitbook.toolbar.removeButtons(buttonsId);
    buttonsId = [];

    // Update code snippets elements
    $codes = $(".api-method-sample");

    // Recreate languages buttons
    var languages = [],
      hasCurrentLang = false;

    $codes.each(function() {
      var isDefault = false,
        codeLang = $(this).data("lang"),
        codeName = $(this).data("name"),
        exists,
        found;

      // Check if is current language
      if (codeLang == apiLanguageSelector.currentLang) {
        hasCurrentLang = true;
        isDefault = true;
      }

      // Check if already added
      exists = $.grep(languages, function(language) {
        return language.name == codeName;
      });

      found = !!exists.length;

      if (!found) {
        // Add language
        languages.push({
          name: codeName,
          lang: codeLang,
          default: isDefault
        });
      }
    });

    // Set languages in good order
    languages.sort(function(first, second) {
      return second.name.localeCompare(first.name);
    });
    $.each(languages, function(i, language) {
      // Set first (last in array) language as active if no default
      var isDefault =
          language.default || (!hasCurrentLang && i == languages.length - 1),
        buttonId;

      // Create button
      buttonId = gitbook.toolbar.createButton({
        text: language.name,
        position: "right",
        className: "lang-switcher" + (isDefault ? " active" : ""),
        onClick: function(e) {
          // Update language
          apiLanguageSelector.currentLang = language.lang;
          saveSettings();

          // Update active button
          $(".btn.lang-switcher.active").removeClass("active");
          $(e.currentTarget).addClass("active");
        }
      });

      // Add to list of buttons
      buttonsId.push(buttonId);

      // Set as current language if is default
      if (isDefault) {
        apiLanguageSelector.currentLang = language.lang;
      }
    });
  }

  // Initialization
  gitbook.events.bind("start", function(e, config) {
    var opts = config["api-language-selector"];

    // Initialize themes
    gitbook.fontsettings.setThemes(THEMES);

    // Set to configured theme
    gitbook.fontsettings.setTheme(opts.theme);

    // Init current settings
    init(opts);
  });

  // Update state
  gitbook.events.on("page.change", function() {
    updateCodeTabs();
    // updateComments();
    updateDisplay();
  });
});
